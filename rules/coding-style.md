...
- Prefer readable single-pass array transforms when possible. Avoid nested iterator chains (for example `map` inside `map`/`flatMap`) when an equally readable single-pass alternative (`reduce`, `forEach` + accumulator, or a clearer flatten-first approach) exists.
- Readability beats micro-optimization by default. If the more performant form is noticeably harder to read, prefer the clearer version unless the user explicitly asks for performance-first code.
